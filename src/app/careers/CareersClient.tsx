...
export default function CareersClient({ initialPositions }: CareersClientProps) {
  const [positionsList, setPositionsList] = useState(initialPositions && initialPositions.length > 0 ? initialPositions : positions);
  const [formData, setFormData] = useState({
    name: "",
    email: "",
    position: (initialPositions && initialPositions[0]?.id) || "1",
    portfolio: "",
    resume: "",
  });
