...
export default function AboutClient({ initialRoadmap, initialSiteSettings }: AboutClientProps) {
  const [roadmap, setRoadmap] = useState(initialRoadmap && initialRoadmap.length > 0 ? initialRoadmap : DEFAULT_ROADMAP_STEPS);
  const [siteSettings, setSiteSettings] = useState<any>(initialSiteSettings || null);

  useEffect(() => {
    if (!initialRoadmap || initialRoadmap.length === 0) {
  ...
  }, [roadmap.length, initialRoadmap, initialSiteSettings]);
