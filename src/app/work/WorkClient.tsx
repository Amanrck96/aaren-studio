...
function mapWorkProjects(projects?: any[]) {
  if (!projects || !Array.isArray(projects) || projects.length === 0) return ALL_PROJECTS;
  return projects.map((p: any, idx: number) => ({
    ...
    image: p.imageUrl || p.image || "https://images.unsplash.com/photo-1618221195710-dd6b41faaea6?auto=format&fit=crop&w=800&q=80",
...
export default function WorkClient({ initialProjects }: WorkClientProps) {
  const [projectsList, setProjectsList] = useState(() => mapWorkProjects(initialProjects));
    ...
    fetch("/api/projects?t=" + Date.now(), { cache: "no-store" })
      ...
      .then((json) => {
        if (json && json.success && Array.isArray(json.data) && json.data.length > 0) {
          setProjectsList(mapWorkProjects(json.data));
        }
      })
