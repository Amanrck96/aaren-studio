...
type Props = {
  ...
  initialAllProducts: ProductItem[];
};

export default function ProductDetailClient({ slug, initialProduct, initialAllProducts }: Props) {
  // Core States
  const [product, setProduct] = useState<ProductItem | null>(initialProduct);
  const [allProducts, setAllProducts] = useState<ProductItem[]>(initialAllProducts && initialAllProducts.length > 0 ? initialAllProducts : DEFAULT_PRODUCTS);
  ...
  // Fetch product & all catalog data only if initialProduct was not provided
  useEffect(() => {
    try {
