...
function mapCategoriesList(cats?: any[]) {
  if (!cats || !Array.isArray(cats) || cats.length === 0) return HOME_CATEGORIES;
...
function mapBrandsList(brands?: any[]) {
  if (!brands || !Array.isArray(brands) || brands.length === 0) return HOME_BRANDS;
  const norm = (s: string) => (s || "").toLowerCase().replace(/[^a-z0-9]/g, "");
...
function mapProjectsList(projects?: any[]) {
...
interface HomeClientProps {
  initialSettings?: SiteSettingsItem | null;
...
export default function HomeClient({
  ...
  const [siteSettings, setSiteSettings] = useState<SiteSettingsItem>(initialSettings || DEFAULT_SETTINGS);
  const [categoriesList, setCategoriesList] = useState(() => mapCategoriesList(initialCategories));
  const [brandsList, setBrandsList] = useState(() => mapBrandsList(initialBrands));
  const [projectsList, setProjectsList] = useState(() => mapProjectsList(initialProjects));

  useEffect(() => {
    // Only client re-fetch if initial server data wasn't provided
    if (initialSettings && initialCategories?.length && initialBrands?.length && initialProjects?.length) {
      return;
    }
    ...
    Promise.all([
      ...
      if (catsJson?.success && Array.isArray(catsJson.data) && catsJson.data.length > 0) {
        setCategoriesList(mapCategoriesList(catsJson.data));
      }
      if (brandsJson?.success && Array.isArray(brandsJson.data) && brandsJson.data.length > 0) {
        setBrandsList(mapBrandsList(brandsJson.data));
      }
      if (projectsJson?.success && Array.isArray(projectsJson.data) && projectsJson.data.length > 0) {
        setProjectsList(mapProjectsList(projectsJson.data));
      }
    });
  }, [initialSettings, initialCategories, initialBrands, initialProjects]);
