...
export default function CategoryShowroom({
  ...
  const pageTagline = isHub
    ...
    : currentCategory.tagline || currentCategory.description || "Official Catalogues & Specifications";
  ...
  const logoImage = isHub
    ? DEFAULT_LOGO
    : currentCategory.logoUrl || currentCategory.bannerImageUrl || DEFAULT_LOGO;

  const files: CategoryFolderPdf[] = currentCategory
    ? (currentCategory.files || []).slice().sort((a, b) => (a.order || 0) - (b.order || 0))
    : [];
      ...
      if (navigator.share) {
        try {
          await navigator.share({
            title: `${pageTitle} | Official Catalogues`,
            text: pageTagline,
            url,
          });
          return;
  ...
  return (
    ...
    >
      {/* Centered shell — max 480px, mobile-first luxury card layout */}
      <div
        style={{
          width: "100%",
          maxWidth: "480px",
      ...
      >
        {/* ── Top Bar Navigation (Same as Brands) ── */}
        <div
          style={{
            width: "100%",
            display: "flex",
        ...
        {/* ══════════════════════════════════════
            1. HERO PROFILE CARD (EXACT BRANDS WAVE LAYOUT)
            ══════════════════════════════════════ */}
        <header
          style={{
            position: "relative",
        ...
        >
          ...
          {/* Circular Category Logo Badge (left: 16px, top: 295px — exact brand style) */}
          <div
            style={{
              position: "absolute",
              left: "16px",
          ...
          >
            ...
            >
              {logoImage ? (
                ...
                <Layers style={{ width: 44, height: 44, color: "#81663F" }} />
              )}
            </div>
          </div>

          {/* Lower Brown Profile Card Content */}
          <div
            style={{
            ...
            >
              ...
              <span>
                {isHub
                  ...
                  : `${files.length} ${files.length === 1 ? "Official Catalogue" : "Official Catalogues"}`}
              </span>
            </div>
          </div>
        </header>
        ...
        {/* ══════════════════════════════════════
            2. BODY CONTENT: HUB VS CATEGORY SHOWROOM
            ══════════════════════════════════════ */}
        {isHub ? (
          /* ── HUB VIEW: SEARCH & CATEGORY LIST ── */
          <div style={{ width: "100%", marginTop: "14px", display: "flex", flexDirection: "column", gap: "12px" }}>
            ...
            {filteredCategories.length > 0 ? (
              filteredCategories.map((c) => {
                ...
                const thumbSrc = c.logoUrl || c.bannerImageUrl || DEFAULT_LOGO;
                return (
                  <Link
                    key={c.id || c.slug}
          ...
          /* ── CATEGORY SHOWROOM: PDF CATALOGUES (SAME TO SAME AS BRANDS) ── */
          <div style={{ width: "100%", marginTop: "14px", display: "flex", flexDirection: "column", gap: "12px" }}>
            {files.length > 0 ? (
              files.map((file, idx) => {
                  ...
                  >
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: "14px", minWidth: 0, flex: 1 }}>
                      ...
                      <div style={{ minWidth: 0, flex: 1 }}>
                        ...
                        <p style={{ fontSize: "12px", color: "#8A8275", marginTop: "2px", margin: "2px 0 0" }}>
                          {file.fileSize || "PDF Document"}
                        </p>
                      </div>
                    </div>

                    {/* Actions: BOTH View ExternalLink AND Download (EXACT BRAND STYLE) */}
              ...
              >
                <p style={{ fontSize: "15px", fontWeight: 600, color: "#1E1E1E", margin: 0 }}>
                  Catalogues Updating
                </p>
                <p style={{ fontSize: "12px", color: "#8A8275", marginTop: "6px", margin: "6px 0 0" }}>
                  The latest architectural specifications for {currentCategory?.name} are being prepared.
                </p>
                <Link
                  href="/contact"
                  style={{
            ...
            {/* Custom CTA Buttons (if any) */}
            {currentCategory?.ctaButtons && currentCategory.ctaButtons.length > 0 && (
              <div
                style={{
                  width: "100%",
                  backgroundColor: "#fff",
              ...
              >
                ...
                {currentCategory.ctaButtons.map((cta, i) => (
                  <a
                    key={i}
        ...
        {/* ══════════════════════════════════════
            3. CONTACT CARD (OFFICIAL BENGALURU HEADQUARTERS - EXACT SAME AS BRANDS)
            ══════════════════════════════════════ */}
        <section
          style={{
            width: "100%",
            ...
            >
              <div style={{ display: "flex", alignItems: "flex-start", gap: "10px" }}>
                ...
                <div style={{ minWidth: 0, flex: 1 }}>
                  ...
                  <p style={{ fontSize: "12px", color: "#2A2824", lineHeight: 1.6, marginTop: "2px", margin: "2px 0 0" }}>
        ...
        {/* ══════════════════════════════════════
            4. SOCIAL LINKS CARD (EXACT SAME AS BRANDS)
            ══════════════════════════════════════ */}
        <section
          style={{
            width: "100%",
        ...
        >
          <h2 style={{ fontSize: "16px", fontWeight: 700, color: "#222", margin: "0 0 14px" }}>
            Social Links
          </h2>
        ...
        <footer style={{ marginTop: "24px", textAlign: "center" }}>
          ...
          <p style={{ margin: "4px 0 0", fontSize: "10px", color: "#8A8275" }}>
